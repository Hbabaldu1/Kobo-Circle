...
export default async function ProfilePage() {
  ...
  const [{ data: state }, { data: lga }, { data: ward }, { data: trust }, { data: listings }, { data: preferences }] = await Promise.all([
    supabase.from('states').select('name').eq('id', profile.state_id).maybeSingle(), supabase.from('lgas').select('name').eq('id', profile.lga_id).maybeSingle(), profile.ward_id ? supabase.from('wards').select('name').eq('id', profile.ward_id).maybeSingle() : Promise.resolve({ data: null }), supabase.from('seller_trust').select('community_vouch_count,tenure_vouch_count,transaction_vouch_count,weighted_score,trust_ratio').eq('user_id', user.id).maybeSingle(), supabase.from('listings').select('id,title,status').eq('user_id', user.id).order('created_at', { ascending: false }), supabase.from('notification_preferences').select('vouch_enabled,message_enabled,listing_enabled').eq('user_id', user.id).maybeSingle(),
  ]);
  const percentage = Math.round(Number(trust?.trust_ratio ?? 0) * 100);
  const communityVouches = Number(trust?.community_vouch_count ?? 0);
  const tenureVouches = Number(trust?.tenure_vouch_count ?? 0);
  const transactionVouches = Number(trust?.transaction_vouch_count ?? 0);
  const totalVouches = communityVouches + tenureVouches + transactionVouches;
  const memberSince = new Intl.DateTimeFormat(undefined, { month: 'long', year: 'numeric' }).format(new Date(profile.created_at));
  return <main className="page-transition mx-auto min-h-screen max-w-lg px-5 py-10"><a href="/feed" className="text-sm font-semibold text-adire">← Back to feed</a>
    ...
    <section className="mt-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><p className="text-xs font-bold uppercase tracking-[.16em] text-adire">Trust summary</p><div className="mt-3 flex items-end justify-between"><div><p className="font-heading text-3xl font-bold text-ink">{percentage}%</p><p className="text-sm text-slate-600">Community trust score</p></div><p className="text-sm font-semibold text-leaf">{totalVouches} {totalVouches === 1 ? 'vouch' : 'vouches'}</p></div><dl className="mt-4 grid grid-cols-3 gap-3 text-center text-sm"><div className="rounded-lg bg-paper p-3"><dt className="text-slate-500">Community</dt><dd className="mt-1 font-bold text-ink">{communityVouches}</dd></div><div className="rounded-lg bg-paper p-3"><dt className="text-slate-500">Tenure</dt><dd className="mt-1 font-bold text-ink">{tenureVouches}</dd></div><div className="rounded-lg bg-paper p-3"><dt className="text-slate-500">Transactions</dt><dd className="mt-1 font-bold text-ink">{transactionVouches}</dd></div></dl></section>
    ...
    <section className="mt-4 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><p className="text-xs font-bold uppercase tracking-[.16em] text-adire">Account actions</p><div className="mt-4"><EditPhoneForm phone={profile.phone} /></div><div className="mt-5"><ThemeToggle /></div><NotificationPreferences initial={preferences ?? { vouch_enabled: true, message_enabled: true, listing_enabled: true }} /><div className="mt-5"><LogoutButton /></div></section>
  </main>;
}
