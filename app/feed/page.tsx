...
export default async function FeedPage({ searchParams }: { searchParams: { posted?: string } }) {
  ...
  return <main className="mx-auto min-h-screen max-w-2xl px-5 lg:max-w-6xl py-10"><div className="flex items-start justify-between gap-4"><LogoutButton /><div><p className="font-heading text-sm font-bold uppercase tracking-[0.18em] text-adire">Kobo Circle</p><h1 className="mt-2 font-heading text-3xl font-bold text-ink">Your estate feed</h1></div><div className="flex shrink-0 gap-2"><a href="/profile" className="rounded-lg border border-adire px-3 py-2 text-sm font-semibold text-adire transition-transform duration-100 active:scale-95 motion-reduce:transition-none motion-reduce:active:scale-100">My Profile</a><a href="/new-listing" className="rounded-lg bg-adire px-3 py-2 text-sm font-semibold text-white transition-transform duration-100 active:scale-95 motion-reduce:transition-none motion-reduce:active:scale-100">Post listing</a></div></div><p className="mt-2 text-sm text-slate-600">Trusted finds and helpful neighbours, close to home.</p>{listings.length ? <FeedList listings={listings} currentUserId={user.id} postedListingId={searchParams.posted ?? null} /> : <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><h2 className="font-heading text-xl font-bold text-ink">Start the conversation</h2><p className="mt-2 leading-6 text-slate-600">There are no listings in your estate yet. Be the first neighbour to share something useful.</p><a href="/new-listing" className="mt-4 inline-block rounded-lg bg-adire px-4 py-3 font-semibold text-white transition-transform duration-100 active:scale-95 motion-reduce:transition-none motion-reduce:active:scale-100">Post a listing</a></section>}</main>;
}
