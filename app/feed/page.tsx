...
export default async function FeedPage() {
  ...
  const { data, error } = await supabase.rpc('listings_with_trust');
  if (error) {
    console.error('Supabase feed query failed.', { message: error.message, code: error.code, details: error.details, hint: error.hint });
    throw new Error('Unable to load the estate feed. Check the server logs for the Supabase error.');
  }
