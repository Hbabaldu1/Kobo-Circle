...
import { vouchTypeSchema } from '@/lib/validation';
...
export async function createVouch(_: VouchActionState, formData: FormData): Promise<VouchActionState> {
  ...
  const parsedVouchType = vouchTypeSchema.safeParse(formData.get('vouchType'));
  if (!parsedVouchType.success) return { error: 'Choose the type of vouch you want to give.' };
  ...
  try {
    ...
    const { error } = await supabase.from('vouches').insert({
      voucher_id: user.id,
      vouched_for_id: sellerId,
      vouch_type: parsedVouchType.data,
    });
    if (error) {
      if (error.code === '23505' || error.message.includes('one_vouch_per_type_per_pair')) return { error: "You've already given this type of vouch to this neighbour." };
      ...
      if (error.code === '23502' || error.code === '23514') return { error: 'Choose a valid type of vouch and try again.' };
