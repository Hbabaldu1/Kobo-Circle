...
export default async function MessagesPage({ searchParams }: MessagesPageProps) {
  ...
  if (selected && counterparty) return <main className="min-h-[100dvh] bg-white"><ChatHeader name={counterparty.name} avatarUrl={counterparty.avatar_url} profileId={counterparty.id} listingId={selectedListing?.id} /><ChatWindow conversationId={selected.id} currentUserId={user.id} initialMessages={selectedMessages ?? []} counterparty={{ id: counterparty.id, name: counterparty.name, avatarUrl: counterparty.avatar_url }} listing={selectedListing ? { id: selectedListing.id, title: selectedListing.title } : undefined} /></main>;
