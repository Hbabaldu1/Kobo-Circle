...
type Lga = { id: string; state_id: string; name: string };
type Ward = { id: string; lga_id: string; name: string };
...
export function OnboardingForm({
  ...
  const availableWards = useMemo(
    () => (lgaId ? wards.filter((w) => w.lga_id === lgaId) : []),
    [wards, lgaId]
  );

  const hasWardData = availableWards.length > 0;
  ...
  async function submit(event: FormEvent) {
    ...
    const validated = onboardingProfileSchema.safeParse({
      name,
      lgaId,
    ...
    if (!validated.success || !stateId || !lgaId || (hasWardData && !wardId)) {
      setError(
        hasWardData
          ? 'Enter your name, then choose a State, Local Government, and Ward.'
          : 'Enter your name, State, and Local Government.'
      );
      return;
    }
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-md items-center px-4 py-8">
      <section className="w-full rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        <p className="font-heading text-sm font-bold uppercase tracking-[0.18em] text-adire">
          Kobo Circle
        </p>
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">
          Tell neighbours where to find you. Add your phone so buyers can reach you on WhatsApp,
          or skip it for now.
        </p>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          <label className="block text-sm font-semibold">
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-3 text-base disabled:bg-slate-100"
            >
              <option value="">Choose your Local Government</option>
              {availableLgas.map((l) => (
          ...
          {lgaId && hasWardData ? (
            ...
            <p className="text-sm text-slate-600">
              Ward-level data isn&apos;t available for this area yet — you can still join using
