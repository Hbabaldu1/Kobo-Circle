...
export function ChatWindow({ conversationId, currentUserId, initialMessages, counterparty, listing }: ChatWindowProps) {
  ...
  useEffect(() => {
    ...
    void supabase
      ...
      .then(({ error: readError }) => {
        if (!readError) setMessages((current) => current.map((message) => message.sender_id === currentUserId || message.read_at ? message : { ...message, read_at: readAt }));
      });
  ...
  return <div className="flex min-h-[calc(100dvh-12rem)] flex-col bg-paper">
    <div className="flex-1 space-y-3 overflow-y-auto px-4 py-5" aria-live="polite">
      <section className="mb-8 flex flex-col items-center rounded-2xl bg-white p-5 text-center shadow-sm ring-1 ring-[#E9DFCF]"><UserAvatar id={counterparty.id} name={counterparty.name} avatarUrl={counterparty.avatarUrl} className="h-24 w-24 text-3xl" /><h2 className="mt-3 font-heading text-xl font-bold text-ink">{counterparty.name}</h2><p className="mt-1 text-sm text-slate-500">Estate neighbour</p><Link href={listing ? `/listings/${listing.id}` : `/sellers/${counterparty.id}`} className="mt-3 rounded-full bg-[#EFE7D6] px-4 py-2 text-sm font-semibold text-adire hover:bg-[#E9DFCF]">{listing ? 'View Listing' : 'View Profile'}</Link></section>
    ...
    <form onSubmit={sendMessage} className="sticky bottom-16 border-t border-[#E9DFCF] bg-white p-3 md:bottom-0">{error && <p role="alert" className="mb-2 text-sm text-brick">{error}</p>}<div className="flex items-center gap-2"><button type="button" aria-label="Attach media" className="rounded-full p-2 text-adire hover:bg-[#EFE7D6]"><NavIcon name="attachment" /></button><textarea value={content} onChange={(event) => setContent(event.target.value)} maxLength={1000} rows={1} placeholder="Aa" className="min-h-10 flex-1 resize-none rounded-full bg-[#EFE7D6] px-4 py-2 text-sm text-ink outline-none ring-adire focus:ring-2" /><button type="submit" aria-label="Send message" disabled={sending || !content.trim()} className="rounded-full bg-adire p-2 text-white disabled:opacity-50"><NavIcon name="send" /></button></div></form>
  </div>;
}
