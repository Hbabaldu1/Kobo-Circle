...
export default async function MessagesPage({ searchParams }: MessagesPageProps) {
  ...
  const [{ data: people }, { data: latestMessages }, { data: selectedMessages }, { data: unreadMessages }, { data: listings }] = await Promise.all([
    ...
    rows.length ? supabase.from('messages').select('conversation_id').in('conversation_id', rows.map((conversation) => conversation.id)).neq('sender_id', user.id).is('read_at', null) : Promise.resolve({ data: [] }),
  ...
  (unreadMessages ?? []).forEach((message) => unreadByConversation.set(message.conversation_id, (unreadByConversation.get(message.conversation_id) ?? 0) + 1));
